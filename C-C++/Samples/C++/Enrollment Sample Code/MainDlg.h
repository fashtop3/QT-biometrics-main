...
public:
	...
	LRESULT OnBnClickedVerification(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
		if (m_RegTemplate.cbData == 0 || m_RegTemplate.pbData == NULL) {
			MessageBox(_T("Before attempting fingerprint verification, you must either select \"Fingerprint Enrollment\" to create a Fingerprint Enrollment Template, or read a previously saved template using \"Read Fingerprint Enrollment Template\"."), _T("Fingerprint Verification"), MB_OK | MB_ICONINFORMATION);
		}
		else {
	...
	LRESULT OnBnClickedSaveTemplate(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
		if (m_RegTemplate.cbData == 0 || m_RegTemplate.pbData == NULL) {
			MessageBox(_T("Before attempting to save a template, select \"Fingerprint Enrollment\" to create a Fingerprint Enrollment Template or select \"Read Fingerprint Enrollment Template\" to read a template."), _T("Save Fingerprint Template"), MB_OK | MB_ICONINFORMATION);
			return 0;
		}
	...
	LRESULT OnBnClickedReadTemplate(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
		...
		try {
			...
			ofn.hwndOwner = m_hWnd;
			ofn.hInstance = _Module.GetResourceInstance();
			ofn.lpstrFilter = _T("Fingerprint Templates(*.fpt)\0*.fpt");
			ofn.lpstrFile = FileNameBuffer;
			ofn.nMaxFile = MAX_PATH;
			ofn.Flags = OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST;
			ofn.lpstrDefExt = _T(".fpt");
